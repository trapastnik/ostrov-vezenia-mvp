...
## Таможенные декларации ПТД-ЭГ ✅ (добавлено 2026-03-04)

| # | Задача | Статус | Заметки |
|---|--------|--------|---------|
| 92 | Модель БД: CustomsDeclaration + CompanySettings | ✅ | Снапшот данных отправителя при создании |
| 93 | FK orders.customs_declaration_id + ALTER TABLE на prod | ✅ | PostgreSQL не поддерживает create_all для ALTER |
| 94 | Сервис: создание, смена статуса, валидация, удаление декларации | ✅ | `customs_declaration.py` |
| 95 | Сервис: обновление таможенных полей товаров (deepcopy + flag_modified) | ✅ | Фикс SQLAlchemy JSON change detection |
| 96 | Экспорт CSV (UTF-8 BOM, `;` разделитель, для таможенного ПО) | ✅ | `customs_export.py` |
| 97 | Экспорт PDF (reportlab + DejaVuSans/Bold для кириллицы) | ✅ | Фикс: шрифт перезалит из dejavu-fonts v2.37 |
| 98 | API: CRUD деклараций + настройки компании | ✅ | `admin_customs.py`, `admin_company.py` |
| 99 | Фронт: список + создание из заказов | ✅ | `CustomsDeclarationsPage.vue` |
| 100 | Фронт: детальная страница (заказы, товары, валидация, CSV/PDF) | ✅ | `CustomsDeclarationDetailPage.vue` |
| 101 | Фронт: редактор таможенных полей товара | ✅ | `CustomsItemEditor.vue` |
| 102 | Фронт: колонка «Декларация» в таблице заказов | ✅ | Номер + статус-бейдж в OrdersListPage |
| 103 | Версия 0.2.1, CI/CD деплой | ✅ | |
...
## ДТЭГ: ТН ВЭД справочник ✅ (добавлено 2026-03-08)

| # | Задача | Статус | Заметки |
|---|--------|--------|---------|
| 110 | Backend: GET /admin/tnved/children — навигация по дереву | ✅ | Lazy loading, has_children, breadcrumbs |
| 111 | Frontend: TnVedBrowser.vue — иерархический каталог (модалка) | ✅ | Группы → позиции → подпозиции, бейджи уровней |
| 112 | Интеграция каталога в CustomsItemEditor (кнопка-папка) | ✅ | Два способа: автоподбор + каталог |
| 113 | Импорт полной базы ТН ВЭД из TWS.BY Excel | ✅ | 17 044 кода (13 289 из TWS + 96 групп + 3 659 промежуточных), актуальность 2026-03-08 |
| 114 | Автогенерация промежуточных узлов дерева (level 2, 4, 6, 8) | ✅ | Справочник GROUP_NAMES для 96 корневых групп |
| 115 | Деплой полной базы на VPS | ✅ | `docker compose exec backend python -m scripts.import_tn_ved data/tn_ved_tws.xlsx` |

---

## Тестовый Bitrix-магазин (добавлено 2026-03-08)
